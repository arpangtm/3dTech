...
const modelPaths = [
  ...
  "/3dmodels/AlienWareLaptop_Low.glb",
    ...
    switch (index % 6) {
      case 0: // Rotate around Y axis based on scroll with enhanced hover effect
        ref.current.rotation.y =
          (rotation?.y || 0) +
          scrollFactor * Math.PI * 2 +
          Math.sin(t * hoverFactor) * 0.3;
        ref.current.position.z =
          position?.z || 0 + (hovered ? Math.sin(t * 2) * 0.2 : 0);
        break;
      case 1: // Move up/down based on scroll with hover bounce
        ref.current.position.y =
          (position?.y || 0) +
          Math.sin(scrollFactor * 3) * 0.5 +
          (hovered ? Math.abs(Math.sin(t * 3)) * 0.3 : 0);
        ref.current.rotation.z =
          (rotation?.z || 0) + (hovered ? Math.sin(t) * 0.1 : 0);
        break;
      ...
      case 3: // Rotate around X axis based on scroll with wobble on hover
        ref.current.rotation.x =
          (rotation?.x || 0) +
          scrollFactor * Math.PI +
          (hovered ? Math.sin(t * 4) * 0.2 : 0);
        ref.current.rotation.z =
          (rotation?.z || 0) + (hovered ? Math.cos(t * 3) * 0.1 : 0);
        break;
      case 4: // Spiral movement
        ref.current.position.x =
          (position?.x || 0) +
          Math.sin(scrollFactor * 2 + t) * (0.3 + (hovered ? 0.2 : 0));
        ref.current.position.z =
          (position?.z || 0) +
          Math.cos(scrollFactor * 2 + t) * (0.3 + (hovered ? 0.2 : 0));
        ref.current.rotation.y =
          (rotation?.y || 0) + t * 0.5 + (hovered ? t : 0);
        break;
      case 5: // Orbit around center
        const orbitRadius = 0.5 + (hovered ? Math.sin(t * 2) * 0.2 : 0);
        ref.current.position.x =
          (position?.x || 0) + Math.sin(scrollFactor + t * 0.5) * orbitRadius;
        ref.current.position.z =
          (position?.z || 0) + Math.cos(scrollFactor + t * 0.5) * orbitRadius;
...
export function AnimatedGadgets({ scrollY }) {
  ...
  useFrame((state) => {
    ...
    group.current.rotation.x =
      Math.sin(t / 15) * 0.05 + Math.sin(scrollFactor * 0.5) * 0.1;
    group.current.position.y =
      Math.sin(t / 8) * 0.1 + Math.sin(scrollFactor) * 0.2;
  });
    ...
    <group ref={group}>
      ...
      {modelPaths.map((path, index) => (
        <Gadget
          ...
          rotation={{
            x: 0,
            y: Math.PI * 2 * (index / modelPaths.length),
            z: 0,
          }}
          scale={scales[index]}
          scrollY={scrollY}
